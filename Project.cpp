...
void vsPlayerGame(bool AisAI, bool BisAI)
{
	...
	do {
		...
		{
			list<Pieces*>::iterator iterA = A.getMovedPieces().begin();
			advance(iterA, 2);
			list<Pieces*>::iterator iterB = B.getMovedPieces().begin();
			advance(iterB, 2);

			
			if (A.getMovedPieces().front()->name == (*iterA)->name && A.getMovedPieces().front()->prevX.front() == (*iterA)->prevX.front() && A.getMovedPieces().front()->prevY.front() == (*iterA)->prevY.front() &&
				B.getMovedPieces().front()->name == (*iterB)->name && B.getMovedPieces().front()->prevX.front() == (*iterB)->prevX.front() && B.getMovedPieces().front()->prevY.front() == (*iterB)->prevY.front())
				drawCounter++;
			else
				drawCounter--;
